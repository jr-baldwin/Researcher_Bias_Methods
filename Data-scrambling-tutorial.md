...
Please first install and load the following packages. If you have already installed them, you can skip to loading them.
...
corDV <- c(0, 0, 0.2, 1) #correlation between each predictor and the outcome. The final 1 is the variance of the outcome, in effect, correlation of variable with itself.
```
...
``` r
scrambled_data <- lapply(sim_data, function(x) { sample(x)})
scrambled_data <- as.data.frame(scrambled_data)
```
...
``` r
head(scrambled_data[order(scrambled_data$id, decreasing= F),], n = 10) # Scrambled dataset
```
